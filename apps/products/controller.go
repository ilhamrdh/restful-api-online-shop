...
func (c Controller) CreateProduct(ctx *fiber.Ctx) error {
	...
	if err := c.service.CreateProduct(ctx.UserContext(), req); err != nil {
...
func (c Controller) GetListProducts(ctx *fiber.Ctx) error {
	...
	products, err := c.service.GetAllProducts(ctx.UserContext(), req)
	if err != nil {
		myErr, ok := response.ErrorMapping[err.Error()]
		if !ok {
			myErr = response.ErrorGeneral
	...
	productListResponse := NewProductResponse(products)
...
func (c Controller) GetProductDetail(ctx *fiber.Ctx) error {
	sku := ctx.Params("sku", "")
	if sku == "" {
		return infrafiber.NewResponse(
			infrafiber.WithMessage("invalid payload"),
	...
	product, err := c.service.ProductDetail(ctx.UserContext(), sku)
	if err != nil {
		myErr, ok := response.ErrorMapping[err.Error()]
		if !ok {
			myErr = response.ErrorGeneral
